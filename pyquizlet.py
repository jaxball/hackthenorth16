...
class Quizlet():
    ...
    def make_request(self, apistring, params={}):
        ...
        if self.authorized and self.access_token:
            ...
            # customHeader = {'Authorization','Bearer ' + self.access_token['access_token']}
            # strHeader = str(customHeader)
            connection.putrequest('GET', '/')
            connection.putheader('Authorization','Bearer ' + self.access_token['access_token'])
            # http = urllib2.PoolManager(threadsNo, maxsize=threadsNo, block=True)
            # request = http.request('GET', queryUrl, preload_content=False)
